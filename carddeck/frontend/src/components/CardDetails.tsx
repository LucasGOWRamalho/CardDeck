...
import { ArrowLeft, CreditCard, Building, DollarSign, Calendar, User, Shield, Plus, TrendingUp, Receipt } from "lucide-react";
...
export function CardDetails({ card, onBack, onCardUpdate }: CardDetailsProps) {
  ...
  useEffect(() => {
    if (card?.id) {
      const storedTransactions = localStorage.getItem(`card-${card.id}-transactions`);
  ...
  // Atualizar o card atual com as transações
  useEffect(() => {
    if (currentCard) {
      const updatedCard = {
        ...currentCard,
  ...
  const handleTransactionAdded = (newTransaction: Transaction) => {
    ...
    if (card.id) {
      localStorage.setItem(`card-${card.id}-transactions`, JSON.stringify(updatedTransactions));
    }
    
    setTransactions(updatedTransactions);
    
    // Atualizar o card com as novas transações
    const updatedCard = {
      ...currentCard,
  ...
  // Calcular limite disponível e saldo atualizado
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">R$ {currentBalance.toLocaleString()}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-semibold">R$ {availableLimit.toLocaleString()}</p>
                </div>
              </div>
